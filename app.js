...
const CONFIG = {
  ...
  // Géopolitique : 6 requêtes GDELT par cycle, cadence plus lente que les news.
  // Allongée : GDELT limite le débit plus strictement que documenté (429 observés
  // en rafale, y compris sur des requêtes isolées et sans lien entre elles).
  GEO_INTERVAL: 900000,
  // Espacement entre deux requêtes GDELT d'un même cycle (news ou géo) : on
  // remplace une rafale simultanée par des appels étalés dans le temps, ce
  // qui est beaucoup moins susceptible de déclencher une limite de débit.
  // 900 ms : suffisant pour ne plus ressembler à une rafale, sans faire
  // traîner un cycle complet de 6 zones sur plus de 5 s.
  GDELT_STAGGER_MS: 900,
  // Si GDELT renvoie 429, on suspend TOUTE requête GDELT (news + géo) pendant
  // ce délai avant de retenter quoi que ce soit — inutile de marteler une
  // source qui vient de nous demander explicitement de ralentir.
  GDELT_429_COOLDOWN_MS: 900000,
  // Durée au-delà de laquelle un instantané restauré depuis le stockage local
  // n'est plus jugé représentatif et laisse la place au contenu de data.json.
  SNAPSHOT_MAX_AGE_MS: 86400000,
  // Fenêtre minimale entre deux échantillons pour juger d'une tendance de zone.
  GEO_TREND_MIN_GAP_MS: 1800000,
  // Volume d'articles (24 h) au-delà duquel une zone est saturée (sévérité 100).
  GEO_VOLUME_SATURATION: 60,
  // Recalibrage des scénarios sur la volatilité réalisée : une fois par jour.
  SCENARIO_RECALC_MS: 86400000,
...
const STATE = {
  ...
  calendarGenerated: 0,
  calendarManual: 0,
  gdeltCooldownUntil: 0,     // horodatage jusqu'auquel GDELT est mis en pause (429)
  gdeltVia: null,            // 'direct' | 'proxy' — par où la dernière lecture a abouti
  diagLog: [],               // journal technique consultable depuis l'app (mobile)
  geoRestoredAt: null,       // horodatage de l'instantané géo restauré au démarrage
  newsRestoredAt: null,
  lastGeoError: null,        // dernier message d'erreur lisible (diagnostic in-app)
  lastNewsError: null,
  lastGeoAt: null,           // dernier cycle géopolitique réussi
  geoLive: 0,                // zones rafraîchies au dernier cycle
  geoHistory: {},            // { zoneId: [{at, severity}] } pour dériver la tendance
  dailyVolPct: null,         // amplitude quotidienne réalisée (%), base des scénarios
  scenarioCalibratedAt: null,// horodatage du dernier recalibrage
  scenarioCalibDay: null,    // jour civil du recalibrage (bascule à minuit)
...
async function fetchGdeltBatch() {
  if (gdeltOnCooldown()) {
    STATE.lastNewsError = 'GDELT en pause (429 récent) — nouvelle tentative dans ' +
      Math.ceil((STATE.gdeltCooldownUntil - Date.now()) / 60000) + ' min';
    return { articles: [], okCount: 0, total: NEWS_QUERIES.length, skipped: true };
  }
  ...
  for (let i = 0; i < NEWS_QUERIES.length; i++) {
    if (gdeltOnCooldown()) break;
    const q = NEWS_QUERIES[i];
    try {
      const json = await gdeltFetch(gdeltUrl(q.q), 8000);
      out.push(...(Array.isArray(json.articles) ? json.articles : []));
      okCount++;
    } catch (err) {
      lastErr = err.message;
      console.warn('[GoldCompass] Requête news échouée (' + q.id + '):', err.message);
    }
    if (i < NEWS_QUERIES.length - 1) await sleep(CONFIG.GDELT_STAGGER_MS);
  }
  STATE.lastNewsError = okCount === 0
    ? (gdeltOnCooldown()
        ? 'GDELT en pause (429) — nouvelle tentative dans ' + Math.ceil((STATE.gdeltCooldownUntil - Date.now()) / 60000) + ' min'
        : (lastErr || 'Échec inconnu'))
    : null;
...
function mergeLiveNews(articles) {
  ...
  persistNewsSnapshot(STATE.data.news);
}
...
/* --------------------------------------------------------------------------
   GÉOPOLITIQUE LIVE — sévérité dérivée de la couverture médiatique GDELT

   Principe : on ne dispose d'aucun flux public gratuit qui note l'escalade
   d'un conflit. En revanche, le VOLUME de couverture d'une zone et la densité
   de vocabulaire d'escalade dans les titres sont des proxys mesurables et
   reproductibles. C'est une heuristique assumée, pas une évaluation experte.

   Sévérité = 60 % volume normalisé + 40 % densité de termes d'escalade.
   Tendance = comparaison avec notre propre échantillon précédent (>= 30 min),
   jamais une affirmation du fournisseur.
   -------------------------------------------------------------------------- */
const GEO_ZONES = [
  { id: 'hormuz', zone: "Détroit d'Ormuz", type: 'blocage_maritime',
    query: '("Strait of Hormuz" OR "Iran navy" OR "Iranian tanker" OR "Gulf shipping")' },
  { id: 'ukraine', zone: 'Ukraine / Russie', type: 'conflit_arme',
    query: '(Ukraine AND (strike OR missile OR drone OR offensive OR sanctions))' },
  { id: 'israel_gaza', zone: 'Israël / Moyen-Orient', type: 'conflit_arme',
    query: '((Israel OR Gaza OR Lebanon OR Hezbollah) AND (strike OR ceasefire OR escalation OR rocket))' },
  { id: 'taiwan', zone: 'Taïwan / Chine', type: 'tension_militaire',
    query: '(Taiwan AND (China OR PLA OR incursion OR "military drill" OR blockade))' },
  { id: 'redsea', zone: 'Mer Rouge', type: 'blocage_maritime',
    query: '("Red Sea" AND (Houthi OR attack OR shipping OR vessel))' },
  { id: 'korea', zone: 'Corée du Nord', type: 'tension_militaire',
    query: '("North Korea" AND (missile OR nuclear OR launch OR provocation))' }
];

/* Vocabulaire d'escalade / désescalade, pondéré. */
const ESCALATION_TERMS = [
  { w: 3.0, re: /\b(nuclear|invasion|invade|war declar|martial law)\b/i },
  { w: 2.4, re: /\b(airstrike|air strike|missile|bombard|shelling|offensive)\b/i },
  { w: 2.0, re: /\b(blockade|embargo|seiz(e|ed|ure)|closure|shut(down)?)\b/i },
  { w: 1.8, re: /\b(mobiliz|troops deploy|reinforce|scrambl)\b/i },
  { w: 1.6, re: /\b(sanction|retaliat|escalat|threat(en)?)\b/i },
  { w: 1.4, re: /\b(drone|attack|clash|strike|killed|casualt)\b/i },
  { w: 1.2, re: /\b(cyberattack|sabotage|assassinat|coup)\b/i }
];
const DEESCALATION_TERMS = [
  { w: 2.2, re: /\b(ceasefire|truce|peace deal|withdraw|de-escalat|deescalat)\b/i },
  { w: 1.6, re: /\b(talks|negotiat|agreement|accord|resume|release)\b/i },
  { w: 1.2, re: /\b(calm|ease|easing|restraint|diplomat)\b/i }
];

function scoreTerms(text, table) {
  let s = 0;
  for (const t of table) if (t.re.test(text)) s += t.w;
  return s;
}

/* Vrai si GDELT est en cooldown suite à un 429 récent : on n'émet alors
   strictement aucune requête, ni news ni géo, jusqu'à expiration. */
/* Journal technique visible dans l'app : sur mobile il n'y a pas de console,
   et un échec réseau invisible est un échec qu'on ne peut pas corriger. */
function diag(level, msg) {
  STATE.diagLog.unshift({ at: new Date().toISOString(), level, msg: String(msg).slice(0, 300) });
  STATE.diagLog = STATE.diagLog.slice(0, 60);
}

function gdeltOnCooldown() {
  return Date.now() < STATE.gdeltCooldownUntil;
}
function triggerGdeltCooldown(reason) {
  STATE.gdeltCooldownUntil = Date.now() + CONFIG.GDELT_429_COOLDOWN_MS;
  console.warn('[GoldCompass] GDELT en pause ' + Math.round(CONFIG.GDELT_429_COOLDOWN_MS / 60000) +
               ' min suite à : ' + reason);
}
function sleep(ms) { return new Promise(r => setTimeout(r, ms)); }

/* Lecture GDELT avec repli proxy.
   Le 429 observé est une limite de DÉBIT par adresse IP, pas un blocage CORS :
   passer par un relais change l'IP sortante et a donc une chance réelle de
   passer. On ne déclenche la pause globale que si le direct ET le relais
   échouent tous deux en 429 — sinon on pénaliserait l'app pour rien.

   Note : « chance réelle » n'est pas « garantie ». Si le relais est lui-même
   saturé ou déjà limité, l'échec est simplement signalé comme tel. */
async function gdeltFetch(url, timeout) {
  const attempts = [
    { u: url, via: 'direct' },
    { u: 'https://corsproxy.io/?url=' + encodeURIComponent(url), via: 'proxy' }
  ];
  let saw429 = false, lastErr = null;

  for (const a of attempts) {
    try {
      const res = await fetchWithTimeout(a.u, timeout);
      if (res.status === 429) { saw429 = true; lastErr = new Error('HTTP 429 (' + a.via + ')'); continue; }
      if (!res.ok) { lastErr = new Error('HTTP ' + res.status + ' (' + a.via + ')'); continue; }
      const txt = (await res.text()).replace(/^\uFEFF/, '');   // GDELT peut préfixer un BOM
      if (!txt.trim().startsWith('{')) { lastErr = new Error('Réponse non JSON (' + a.via + ')'); continue; }
      STATE.gdeltVia = a.via;
      return JSON.parse(txt);
    } catch (err) {
      lastErr = err;
      // Une requête bloquée par CORS lève un TypeError « Failed to fetch »
      // indiscernable d'une panne réseau : on journalise le type exact.
      diag('err', 'GDELT ' + a.via + ' : ' + (err?.name || 'Error') + ' — ' + (err?.message || err));
    }
  }
  // Épuisé : direct et relais ont échoué.
  if (saw429) triggerGdeltCooldown('429 en direct ET via relais');
  throw lastErr || new Error('Échec GDELT inconnu');
}

function geoZoneUrl(query) {
  return GDELT_BASE + '?query=' + encodeURIComponent(query) +
         '&mode=artlist&format=json&maxrecords=50&timespan=24h&sort=datedesc';
}

async function fetchGeoZone(z) {
  const json = await gdeltFetch(geoZoneUrl(z.query));
  const arts = Array.isArray(json.articles) ? json.articles : [];

  let esc = 0, des = 0, latest = null, headline = null;
  arts.forEach(a => {
    const t = a.title || '';
    esc += scoreTerms(t, ESCALATION_TERMS);
    des += scoreTerms(t, DEESCALATION_TERMS);
    const d = parseGdeltDate(a.seendate);
    if (d && (!latest || d > latest)) { latest = d; headline = t; }
  });

  // Volume normalisé (0-1) puis densité d'escalade par article (0-1).
  const volume = Math.min(1, arts.length / CONFIG.GEO_VOLUME_SATURATION);
  const perArticle = arts.length ? (esc - des * 0.8) / arts.length : 0;
  const intensity = Math.max(0, Math.min(1, perArticle / 2.5));
  const severity = Math.round(Math.min(100, (volume * 0.6 + intensity * 0.4) * 100));

  return {
    id: z.id, zone: z.zone, type: z.type,
    severity,
    articles: arts.length,
    escalationScore: +esc.toFixed(1),
    deescalationScore: +des.toFixed(1),
    headline: headline ? headline.slice(0, 160) : null,
    last_update: (latest || new Date()).toISOString(),
    live: true
  };
}

/* Tendance : comparaison avec notre propre échantillon antérieur.
   Sans historique suffisant on affiche "stable" plutôt que d'inventer. */
function deriveZoneTrend(id, severity) {
  const hist = STATE.geoHistory[id] || [];
  const now = Date.now();
  const ref = hist.filter(p => now - p.at >= CONFIG.GEO_TREND_MIN_GAP_MS).pop();
  hist.push({ at: now, severity });
  // On conserve 24 h d'échantillons au maximum.
  STATE.geoHistory[id] = hist.filter(p => now - p.at <= 86400000).slice(-48);
  persistGeoHistory();
  if (!ref) return 'stable';
  const delta = severity - ref.severity;
  if (delta >= 8) return 'escalade';
  if (delta <= -8) return 'desescalade';
  return 'stable';
}

/* --------------------------------------------------------------------------
   PERSISTANCE DES INSTANTANÉS LIVE
   Sans cela, chaque rechargement de page repart du contenu figé de data.json :
   l'utilisateur revoit de vieilles données mock alors que l'app avait déjà
   reçu du live lors d'une session précédente. On restaure donc le dernier
   instantané réussi, marqué comme restauré et daté honnêtement — jamais
   présenté comme frais.
   -------------------------------------------------------------------------- */
const GEO_SNAPSHOT_KEY = 'goldcompass.geosnapshot.v1';
const NEWS_SNAPSHOT_KEY = 'goldcompass.newssnapshot.v1';

function persistGeoSnapshot(zones) {
  try {
    window.localStorage?.setItem(GEO_SNAPSHOT_KEY,
      JSON.stringify({ at: Date.now(), zones }));
  } catch { /* stockage indisponible */ }
}
function restoreGeoSnapshot(data) {
  try {
    const raw = window.localStorage?.getItem(GEO_SNAPSHOT_KEY);
    if (!raw) return false;
    const snap = JSON.parse(raw);
    if (!snap?.zones?.length) return false;
    if (Date.now() - snap.at > CONFIG.SNAPSHOT_MAX_AGE_MS) return false;
    // Restauré ≠ live : on le marque explicitement pour que l'UI le dise.
    data.geopolitics = snap.zones.map(z => ({ ...z, live: false, restored: true }));
    STATE.geoRestoredAt = snap.at;
    return true;
  } catch { return false; }
}

function persistNewsSnapshot(items) {
  try {
    const live = (items || []).filter(n => n.live).slice(0, 40);
    if (!live.length) return;
    window.localStorage?.setItem(NEWS_SNAPSHOT_KEY,
      JSON.stringify({ at: Date.now(), items: live }));
  } catch { /* stockage indisponible */ }
}
function restoreNewsSnapshot(data) {
  try {
    const raw = window.localStorage?.getItem(NEWS_SNAPSHOT_KEY);
    if (!raw) return false;
    const snap = JSON.parse(raw);
    if (!snap?.items?.length) return false;
    if (Date.now() - snap.at > CONFIG.SNAPSHOT_MAX_AGE_MS) return false;
    const existing = new Set((data.news || []).map(n => newsKey(n.title, n.url)));
    snap.items.forEach(n => {
      if (!existing.has(newsKey(n.title, n.url))) data.news.push({ ...n, restored: true });
    });
    STATE.newsRestoredAt = snap.at;
    return true;
  } catch { return false; }
}

function persistGeoHistory() {
  try { window.localStorage?.setItem('goldcompass.geohistory.v1', JSON.stringify(STATE.geoHistory)); }
  catch { /* stockage indisponible : l'historique reste en mémoire */ }
}
function loadGeoHistory() {
  try {
    const raw = window.localStorage?.getItem('goldcompass.geohistory.v1');
    STATE.geoHistory = raw ? JSON.parse(raw) : {};
  } catch { STATE.geoHistory = {}; }
}

async function fetchGeoSnapshot() {
  if (gdeltOnCooldown()) {
    STATE.lastGeoError = 'GDELT en pause (429 récent) — nouvelle tentative dans ' +
      Math.ceil((STATE.gdeltCooldownUntil - Date.now()) / 60000) + ' min';
    return { zones: STATE.data?.geopolitics || [], okCount: 0, total: GEO_ZONES.length, skipped: true };
  }

  // Séquentiel et espacé plutôt qu'une rafale de 6 appels simultanés :
  // beaucoup moins susceptible de déclencher une limite de débit.
  const zones = [];
  let ok = 0;
  let lastErr = null;
  for (let i = 0; i < GEO_ZONES.length; i++) {
    if (gdeltOnCooldown()) break; // un 429 en cours de boucle stoppe le reste immédiatement
    try {
      const z = await fetchGeoZone(GEO_ZONES[i]);
      z.trend = deriveZoneTrend(z.id, z.severity);
      z.desc_short = z.headline ||
        (z.articles + ' article' + (z.articles > 1 ? 's' : '') + ' sur 24 h');
      zones.push(z); ok++;
    } catch (err) {
      lastErr = err.message;
      console.warn('[GoldCompass] Zone géopolitique indisponible (' + GEO_ZONES[i].id + '):', err.message);
      const prev = (STATE.data?.geopolitics || []).find(g => g.id === GEO_ZONES[i].id);
      zones.push(prev ? { ...prev, live: false } : { ...GEO_ZONES[i], severity: 0, trend: 'stable', desc_short: 'Indisponible', last_update: new Date().toISOString(), live: false });
    }
    if (i < GEO_ZONES.length - 1) await sleep(CONFIG.GDELT_STAGGER_MS);
  }
  if (ok > 0) persistGeoSnapshot(zones);
  STATE.lastGeoError = ok === 0
    ? (gdeltOnCooldown()
        ? 'GDELT en pause (429) — nouvelle tentative dans ' + Math.ceil((STATE.gdeltCooldownUntil - Date.now()) / 60000) + ' min'
        : (lastErr || 'Échec inconnu'))
    : null;
  return { zones, okCount: ok, total: GEO_ZONES.length };
}
...
/* --------------------------------------------------------------------------
   8. SCENARIO ENGINE
   -------------------------------------------------------------------------- */
/* Amplitudes de scénario exprimées en FRACTION de l'amplitude quotidienne
   réalisée, et non en pourcentages figés. Racine du temps pour les horizons
   pluri-journaliers (48 h ~ sqrt(2), semaine ~ sqrt(7)). */
const HORIZONS = [
  { key: '1h',      label: 'PROCHAINE HEURE',   decay: 0,    volMult: 0.16, hours: 1 },
  { key: 'session', label: 'PROCHAINE SESSION', decay: 0.06, volMult: 0.42, hours: 8 },
  { key: '24h',     label: '24 HEURES',         decay: 0.14, volMult: 1.00, hours: 24 },
  { key: '48h',     label: '48 HEURES',         decay: 0.22, volMult: 1.41, hours: 48 },
  { key: 'week',    label: 'SEMAINE',           decay: 0.35, volMult: 2.65, hours: 168 }
];

/* Amplitude quotidienne réalisée sur les 14 dernières séances : moyenne de
   (plus haut - plus bas) / clôture. C'est ce qui rend les cibles de scénario
   cohérentes avec le marché du moment plutôt qu'avec des constantes écrites
   une fois pour toutes. Repli sur 1,1 % si l'historique est indisponible. */
const DEFAULT_DAILY_VOL_PCT = 1.1;

function computeDailyVolatility(points) {
  if (!Array.isArray(points) || !points.length) return null;
  const usable = points.filter(p =>
    typeof p.h === 'number' && typeof p.l === 'number' && typeof p.c === 'number' && p.c > 0);
  if (usable.length < 5) return null;
  const recent = usable.slice(-14);
  const ranges = recent.map(p => ((p.h - p.l) / p.c) * 100);
  const avg = ranges.reduce((a, b) => a + b, 0) / ranges.length;
  return +avg.toFixed(3);
}

/* Le recalibrage est quotidien : il se déclenche au changement de jour civil
   ou après 24 h, jamais à chaque cycle (l'amplitude réalisée ne bouge pas
   d'une minute à l'autre). */
function scenarioNeedsRecalibration() {
  const today = new Date().toISOString().slice(0, 10);
  if (STATE.scenarioCalibDay !== today) return true;
  if (!STATE.scenarioCalibratedAt) return true;
  return Date.now() - STATE.scenarioCalibratedAt > CONFIG.SCENARIO_RECALC_MS;
}

function applyScenarioCalibration(points) {
  const vol = computeDailyVolatility(points);
  STATE.dailyVolPct = vol ?? DEFAULT_DAILY_VOL_PCT;
  STATE.scenarioCalibratedAt = Date.now();
  STATE.scenarioCalibDay = new Date().toISOString().slice(0, 10);
  return STATE.dailyVolPct;
}
...
function computeScenarios(data, bias, risk) {
  ...
  return HORIZONS.map(h => {
    ...
    const dailyVol = STATE.dailyVolPct ?? DEFAULT_DAILY_VOL_PCT;
    const amplitude = dailyVol * h.volMult;
    const upTarget = +(price * (1 + amplitude / 100)).toFixed(2);
    const downTarget = +(price * (1 - amplitude / 100)).toFixed(2);
...
function renderNewsStatus() {
  ...
  setText('news-error', STATE.newsSourcesOk > 0 ? '' : (STATE.lastNewsError || 'Aucune dépêche live reçue depuis l\'ouverture.'));
  setClass('news-error', STATE.newsSourcesOk > 0 ? 'feed-error-line hidden' : 'feed-error-line');
...
function renderScenarios(containerId, keys) {
  if (containerId === 'scenario-cards-full') {
    const v = STATE.dailyVolPct;
    setText('scenario-calib', v != null
      ? `calibré sur ${fmt(v)}%/jour · ${STATE.scenarioCalibDay ?? '—'}`
      : '1h · session · 24h · 48h · semaine');
  }
...
function renderGeopolitics() {
  const geoTotal = STATE.geoTotal || GEO_ZONES.length;
  const geoOk = STATE.lastGeoAt && STATE.geoLive > 0;
  // Trois états distincts, jamais confondus :
  //   live      → données reçues à l'instant
  //   restauré  → vraies données d'une session antérieure, datées honnêtement
  //   STATIQUE  → contenu de data.json, aucune donnée réelle reçue
  let geoLabel, geoCls;
  if (geoOk) { geoLabel = `${STATE.geoLive}/${geoTotal} live`; geoCls = ''; }
  else if (STATE.geoRestoredAt) {
    geoLabel = 'restauré · ' + timeAgo(new Date(STATE.geoRestoredAt).toISOString());
    geoCls = ' feed-stale';
  } else { geoLabel = 'STATIQUE'; geoCls = ' feed-down'; }
  setText('geo-live-count', geoLabel);
  setClass('geo-live-count', 'section-sub' + geoCls);
  setText('geo-error', geoOk ? '' : (STATE.lastGeoError || 'Aucune donnée live reçue depuis l\'ouverture.'));

  // Journal technique : dernière cause réelle, lisible sans console.
  setText('diag-count', STATE.diagLog.length ? STATE.diagLog.length + ' entrées' : 'aucune');
  const diagSig = STATE.diagLog.length ? STATE.diagLog[0].at + ':' + STATE.diagLog.length : 'empty';
  setHTMLIfChanged('diag-log', diagSig, () => STATE.diagLog.length
    ? STATE.diagLog.slice(0, 25).map(e =>
        `<div class="diag-row ${e.level}"><span class="diag-time">${hhmm(e.at)}</span><span>${e.msg}</span></div>`).join('')
    : '<div class="diag-empty">Aucun incident enregistré.</div>');
  setClass('geo-error', geoOk ? 'feed-error-line hidden' : 'feed-error-line' + (STATE.geoRestoredAt ? ' warn' : ''));
...
async function geoCycle() {
  if (!STATE.paused) {
    try {
    const { zones, okCount, total } = await fetchGeoSnapshot();
    if (okCount > 0) {
      STATE.data.geopolitics = zones;
      STATE.geoLive = okCount;
      STATE.geoTotal = total;
      STATE.lastGeoAt = Date.now();
    }
    recomputeAndRender();   // y compris en échec : le statut doit se mettre à jour
    } catch (err) {
      STATE.lastGeoError = 'Exception : ' + (err?.message || String(err));
      console.error('[GoldCompass] Exception cycle géopolitique :', err);
      recomputeAndRender();
    }
  }
  STATE.timerGeo = setTimeout(geoCycle, CONFIG.GEO_INTERVAL);
}
...
async function dataCycle() {
  if (!STATE.paused) {
    ...
    if (refAge > CONFIG.DAILY_REF_INTERVAL) {
      ...
      if (ref) {
        STATE.dailyRef = ref;
        if (scenarioNeedsRecalibration()) applyScenarioCalibration(ref.points);
        recomputeAndRender();
      }
    }
    // Bascule de jour civil : les scénarios se recalibrent même si
    // l'historique n'a pas eu besoin d'être rechargé.
    if (scenarioNeedsRecalibration() && STATE.dailyRef?.points) {
      applyScenarioCalibration(STATE.dailyRef.points);
      recomputeAndRender();
    }
  }
...
function stopLiveLoop() {
  ...
  clearTimeout(STATE.timerNews); clearTimeout(STATE.timerGeo);
  clearTimeout(STATE.timerData); clearInterval(STATE.timerClock);
  STATE.timerPrice = STATE.timerMacro = STATE.timerNews =
    STATE.timerGeo = STATE.timerData = STATE.timerClock = null;
}

/* Rafraîchissement immédiat, hors cadence (retour d'arrière-plan, tap manuel) */
async function forceRefresh() {
  // Prix et macro d'abord : rapides, c'est ce que l'utilisateur attend en
  // tapant sur STATUT. News et géopolitique espacent chacune leurs propres
  // requêtes en interne (anti-429) — aucune des deux n'est "rapide" — donc
  // toutes deux continuent en tâche de fond après ce premier rendu.
  ...
  fetchGdeltBatch().then(newsBatch => {
    STATE.newsSourcesOk = newsBatch.okCount;
    if (newsBatch.articles.length) mergeLiveNews(newsBatch.articles);
    recomputeAndRender();
  }).catch(err => {
    STATE.lastNewsError = 'Exception : ' + (err?.message || String(err));
    console.error('[GoldCompass] Exception flux news :', err);
    recomputeAndRender();
  });
  fetchGeoSnapshot().then(geoBatch => {
    if (geoBatch.okCount > 0) {
      STATE.data.geopolitics = geoBatch.zones;
      STATE.geoLive = geoBatch.okCount;
      STATE.geoTotal = geoBatch.total;
      STATE.lastGeoAt = Date.now();
    }
    recomputeAndRender();
  }).catch(err => {
    STATE.lastGeoError = 'Exception : ' + (err?.message || String(err));
    console.error('[GoldCompass] Exception flux géopolitique :', err);
    recomputeAndRender();
  });
}
...
async function init() {
  ...
  // Instantanés de la session précédente : mieux vaut du live daté d'il y a
  // deux heures, affiché comme tel, que le contenu figé de data.json présenté
  // comme s'il était actuel.
  restoreGeoSnapshot(STATE.data);
  restoreNewsSnapshot(STATE.data);
  ...
  // Calibrage des scénarios sur l'amplitude quotidienne réellement observée.
  applyScenarioCalibration(STATE.dailyRef?.points);
  ...
  // News et géopolitique continuent EN ARRIÈRE-PLAN, sans bloquer le premier
  // affichage. Les DEUX flux GDELT espacent désormais leurs propres requêtes
  // en interne (anti-429) — ni l'un ni l'autre n'est donc "rapide" : aucun
  // des deux ne doit faire partie du chemin qui retire l'écran de chargement.
  // Le Snapshot s'affiche avec le contenu de data.json en placeholder,
  // remplacé dès que chaque lot live arrive (indépendamment l'un de l'autre).
  fetchGdeltBatch().then(firstNews => {
    STATE.newsSourcesOk = firstNews.okCount;
    if (firstNews.articles.length) mergeLiveNews(firstNews.articles);
    recomputeAndRender();
  }).catch(err => {
    // Sans ce catch, toute exception ici disparaissait sans laisser de trace :
    // l'UI affichait le message générique au lieu de la cause réelle.
    STATE.lastNewsError = 'Exception : ' + (err?.message || String(err));
    console.error('[GoldCompass] Exception flux news :', err);
    recomputeAndRender();
  });
  fetchGeoSnapshot().then(firstGeo => {
    if (firstGeo.okCount > 0) {
      STATE.data.geopolitics = firstGeo.zones;
      STATE.geoLive = firstGeo.okCount;
      STATE.geoTotal = firstGeo.total;
      STATE.lastGeoAt = Date.now();
    }
    // Rendu inconditionnel : en cas d'échec total, c'est précisément là que
    // le badge et le message de diagnostic doivent apparaître.
    recomputeAndRender();
  }).catch(err => {
    STATE.lastGeoError = 'Exception : ' + (err?.message || String(err));
    console.error('[GoldCompass] Exception flux géopolitique :', err);
...
// Filet de sécurité : tout rejet non géré est capturé et journalisé plutôt
// que de disparaître dans la console d'un téléphone que personne n'ouvrira.
window.addEventListener('unhandledrejection', ev => {
  diag('err', 'Rejet non géré : ' + (ev.reason?.message || ev.reason));
});
window.addEventListener('error', ev => {
  diag('err', 'Erreur JS : ' + (ev.message || ev.error));
});
