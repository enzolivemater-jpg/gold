...
const CONFIG = {
  ...
  // Cadence de rechargement du fichier de données complet (macro/news/géo).
  DATA_INTERVAL: 60000,
...
const STATE = {
  ...
  timerPrice: null, timerData: null, timerClock: null,
  biasHistory: [],
...
function renderClockAndFreshness() {
  ...
  const ref = STATE.lastPriceAt ?? STATE.lastDataAt;
  const ageMs = ref ? Date.now() - ref : null;
  ...
  if (dot) {
    const cls = 'live-dot' +
      (STATE.paused ? ' paused' : '') +
      (STATE.liveStatus === 'DEGRADE' || STATE.consecutiveFailures > 0 ? ' failing' : '');
...
function renderMacroTab() {
  ...
  const sig = values.map(i => `${i.label}:${i.value}`).join(',');
...
async function dataCycle() {
  if (!STATE.paused) {
    ...
    if (fresh) {
      // Le prix live déjà obtenu prime sur le prix statique du fichier.
...
function stopLiveLoop() {
  clearTimeout(STATE.timerPrice); clearTimeout(STATE.timerData); clearInterval(STATE.timerClock);
  STATE.timerPrice = STATE.timerData = STATE.timerClock = null;
}
