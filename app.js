...
async function fetchGeoZone(z) {
  const res = await fetchWithTimeout(geoZoneUrl(z.query));
  if (!res.ok) throw new Error('HTTP ' + res.status);
  const text = await res.text();
  // GDELT renvoie parfois un préfixe BOM ou une page d'erreur HTML.
  const clean = text.replace(/^\uFEFF/, '').trim();
  if (!clean.startsWith('{')) throw new Error('Réponse GDELT non-JSON');
...
async function fetchGeoSnapshot() {
  const settled = await Promise.allSettled(GEO_ZONES.map(fetchGeoZone));
  const zones = [];
  let ok = 0;
  settled.forEach((r, i) => {
    if (r.status === 'fulfilled') {
      ...
      console.warn('[GoldCompass] Zone géopolitique indisponible (' +
                   GEO_ZONES[i].id + '):', r.reason?.message);
      // On conserve la dernière valeur connue plutôt que de faire disparaître la zone.
      const prev = (STATE.data?.geopolitics || []).find(g => g.id === GEO_ZONES[i].id);
      if (prev) zones.push({ ...prev, live: false });
    }
  });
...
function renderGeopolitics() {
  ...
  setText('geo-live-count', STATE.lastGeoAt ? `${STATE.geoLive}/${geoTotal} live` : 'statique');
...
async function init() {
  ...
  // News et géopolitique en parallèle : deux flux GDELT indépendants.
  const [firstNews, firstGeo] = await Promise.all([fetchGdeltBatch(), fetchGeoSnapshot()]);
